#!/bin/bash
## written by eden.
## Centos6.5 X86_64 2017-05-31.

#######Begin########
#####Soft_Version#####
mysql_version_5_1="mysql-5.1.72"
mysql_version_5_6="mysql-5.6.35"
apache_version="httpd-2.2.32"
php_version_5_4="php-5.4.45"
php_version_5_6="php-5.6.6"
...
install_mysqld() {
    case $mysql_v in
        5.1)
            cd /usr/local/src
            [ -f $mysql_version_5_1-linux-$ar-glibc23.tar.gz ] || wget http://mirrors.sohu.com/mysql/MySQL-5.1/$mysql_version_5_1-linux-$ar-glibc23.tar.gz
            tar zxf $mysql_version_5_1-linux-$ar-glibc23.tar.gz
            check_ok
        ...
        5.6)
            cd /usr/local/src
            [ -f $mysql_version_5_6-linux-glibc2.5-$ar.tar.gz ] || wget http://mirrors.sohu.com/mysql/MySQL-5.6/$mysql_version_5_6-linux-glibc2.5-$ar.tar.gz
            tar zxf $mysql_version_5_6-linux-glibc2.5-$ar.tar.gz
            check_ok
            [ -d /usr/local/mysql ] && /bin/mv /usr/local/mysql /usr/local/mysql_bak
            mv $mysql_version_5_6-linux-glibc2.5-$ar /usr/local/mysql
            if ! grep '^mysql:' /etc/passwd
            then
                useradd -M mysql -s /sbin/nologin
            fi
...
install_httpd() {
echo "Install apache version $$apache_version"
cd /usr/local/src
[ -f $apache_version.tar.gz ] || wget  https://mirrors.tuna.tsinghua.edu.cn/apache//httpd/$apache_version.tar.gz
tar zxf  $apache_version.tar.gz && cd $apache_version
check_ok
./configure \
...
echo -e "Install php.\nPlease chose the version of php."
    case $php_v in
        5.4)
            cd /usr/local/src/
            [ -f $php_version_5_4.tar.bz2 ] || wget 'http://cn2.php.net/get/$php_version_5_4.tar.bz2/from/this/mirror' -O $php_version_5_4.tar.bz2
            tar jxf $php_version_5_4.tar.bz2 && cd $php_version_5_4
        ...
        5.6)
            cd /usr/local/src/
            [ -f $php_version_5_6.tar.gz ] || wget http://mirrors.sohu.com/php/$php_version_5_6.tar.gz
            tar zxf $php_version_5_6.tar.gz &&   cd $php_version_5_6
            for p in openssl-devel bzip2-devel \
            libxml2-devel curl-devel libpng-devel \
            libjpeg-devel freetype-devel libmcrypt-devel\
            libtool-ltdl-devel perl-devel
...
install_nginx() {
cd /usr/local/src
[ -f $nginx_version.tar.gz ] || wget http://nginx.org/download/$nginx_version.tar.gz
